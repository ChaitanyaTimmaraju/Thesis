...
void ModelLoader::loadObjectAndSetBuffers(QString fileName) {
  ...
  if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    qDebug() << qPrintable(fileName)
             << " not found.(Make sure to add in resources.qrc)";

  // create Index Buffer
  m_indicesID->create();
  m_indicesID->bind();
  m_indicesID->setUsagePattern(QOpenGLBuffer::StaticDraw);
  m_indicesID->allocate(indices.constData(),
  ...
  m_tangentsID.allocate(tangents.constData(),
                        tangents.size() * sizeof(QVector3D));
